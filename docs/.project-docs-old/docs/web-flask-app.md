...
1. **Framework:** The application is built using Flask (`Flask==1.1.2`). This is a relatively old version, as you mentioned.
2. **Database:** It uses SQLAlchemy (`SQLAlchemy==1.4.17`) as an ORM, and the `docker-compose.yml` indicates SQLite is the database (`SQLALCHEMY_DATABASE_URI=${SQLALCHEMY_DATABASE_URI}`). Migrations are handled by `Flask-Migrate` (`Flask-Migrate==3.0.0`) which uses Alembic (`alembic==1.6.5`).
3. **Deployment:** The application is containerized using Docker and orchestrated with `docker-compose`. The `uuebcast` service builds from `./web` and exposes port 8080 (mapped to 8083 on the host).
4. **Configuration:** A significant number of environment variables are used for configuration, loaded from an `.env` file (as indicated by `python-dotenv==0.17.1`). These variables cover Flask settings, database connection, email server details, API keys (Google YouTube, GitHub), and various internal/external paths and URLs.
...
1. User registers and logs in. A default podcast channel and an empty feed file on GitHub are created.
2. User adds a YouTube video URL.
3. The application (using `pytubefix`) fetches video metadata and the audio-only stream.
4. An `Episode` record is created in the SQLite database.
5. The audio is downloaded asynchronously (in a thread) and saved locally (e.g., in `web/app/static/media/VIDEO_ID.m4a`). The `Episode` record is marked as downloaded.
6. A podcast XML feed is generated (using `podgen`) incorporating all the user's episodes and channel details.
7. This XML feed is uploaded/updated on a GitHub repository (using `PyGithub`, also in a thread) specified in the configuration. This leverages GitHub Pages for HTTPS hosting of the feed.
8. Users can manage their channel details (title, description, image) and episodes (delete).
9. A manual sync option allows users to force a regeneration and re-upload of their GitHub feed.
...
- **File Path for `os.path.isfile` in `add_video_episode`:** The check `if os.path.isfile(filepath):` in `add_video_episode` uses `filepath = "{}{}.{}".format(current_app.config['BASE_APP_VIDEO_MEDIA_FILE_PATH'], video.video_id, selected_stream_extension)` when using PytubeFix which seems correct
...
Here are the revised Use Cases from the analysis:
